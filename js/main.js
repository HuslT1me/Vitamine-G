	...
	productItem.hover(
		function () {
			MQ(minLg, ()=> {
				...
				$(this).css('z-index', '3');
				$(this).find('.product__price-container').hide(300)
				...
				MQ(minLg, ()=> {
					...
					$(this).css('z-index', '0');
					$(this).find('.product__price-container').show(300)
...
function catalogProductBreadCrumbs() {
	...
	activeItem.click(function (e) {
		...
		$(this).closest('.product-catalog--popup').toggleClass('product-catalog--active');
		$(this).parent().siblings('.product-catalog__popup-content').toggle(300);
	});
...
function singleProductSlider() {
	...
	$('.product-single-slider').owlCarousel(sliderSettings);
}
